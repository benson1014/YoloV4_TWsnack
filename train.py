...
import os
os.environ['CUDA_VISIBLE_DEVICES']="5"#"0,1,2,3,4,5,6,7"
...
from train_test import (get_train_model, yolo_body, get_lr_scheduler, LossHistory, ModelCheckpoint,
                            YoloDatasets, get_anchors, get_classes, fit_one_epoch)
...
if __name__ == "__main__":
    ...
    eager           = False
    #---------------------------------------------------------------------#
    #   classes_path    指向 model_data 下的txt，與自己訓練的數據集相關 
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    #   權值文件的下載請看 README，可以通過網盤下載。模型的預訓練權重對不同數據集是通用的，因為特徵是通用的。
    #   模型的預訓練權重比較重要的部分是主幹特徵提取網絡的權值部分，用於進行特徵提取。
    #   預訓練權重對於99%的情況都必須要用，不用的話主幹部分的權值太過隨機，特徵提取效果不明顯，網絡訓練的結果也不會好
    #
    #   如果訓練過程中存在中斷訓練的操作，可以將 model_path 設置成 logs 文件夾下的權值文件，將已經訓練了一部分的權值再次載入。
    #   同時修改下方的 凍結階段 或者 解凍階段 的參數，來保證模型epoch的連續性。
    #   
    #   當model_path = ''的時候不加載整個模型的權值。
    #
    #   此處使用的是整個模型的權重，因此是在 train.py 進行加載的。
    #   如果想要讓模型從主幹的預訓練權值開始訓練，則設置model_path為主幹網絡的權值，此時僅加載主幹。
    #   如果想要讓模型從0開始訓練，則設置 model_path = ''，Freeze_Train = Fasle，此時從0開始訓練，且沒有凍結主幹的過程。
    #   
    #   一般來講，網絡從0開始的訓練效果會很差，因為權值太過隨機，特徵提取效果不明顯，因此非常、非常、非常不建議大家從0開始訓練！
    #   從0開始訓練有兩個方案：
    #   1、得益於 Mosaic 數據增強方法強大的數據增強能力，將U nFreeze_Epoch 設置的較大（300及以上）、batch較大（16及以上）、數據較多（萬以上）的情況下，
    #      可以設置mosaic=True，直接隨機初始化參數開始訓練，但得到的效果仍然不如有預訓練的情況。 （像COCO這樣的大數據集可以這樣做）
    #   2、了解imagenet數據集，首先訓練分類模型，獲得網絡的主幹部分權值，分類模型的 主幹部分 和該模型通用，基於此進行訓練。
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    #   mosaic          馬賽克數據增強
    #                   參考YoloX，由於Mosaic生成的訓練圖片，
    #                   遠遠脫離自然圖片的真實分佈。
    #                   本代碼會在訓練結束前的N個epoch自動關掉Mosaic
    #                   100個世代會關閉30個世代（比例可在dataloader.py調整）
    #   label_smoothing 標籤平滑。一般0.01以下。如0.01、0.005
    #
    #   餘弦退火算法的參數放到下面的lr_decay_type中設置
    #------------------------------------------------------------------#
    mosaic              = True
    label_smoothing     = 0
    ...
    #   （三）從0開始訓練：
    #       Init_Epoch = 0，UnFreeze_Epoch >= 300，Unfreeze_batch_size >= 16，Freeze_Train = False（不凍結訓練）
    #       其中：UnFreeze_Epoch盡量不小於300。 optimizer_type = 'sgd'，Init_lr = 1e-2，mosaic = True。
    #   （四）batch_size的設置：
